...
def plot_decision_boundary_2d(
    X_grid, y, prob_function, xx, yy, X_2d, n_features, title=None
):
    """
    ...
    t = "Decision Boundary" if title is None else title
    plt.title(t)
    plt.show()
...
def plot_combined_probability_boundary(
    ...
    legend_elements = [
        plt.Line2D(
            [0], [0], color=color, lw=linewidth, label=f"Class {cls_idx} Boundary"
        )
...
def generate_blobs_dataset(n_samples, n_features, n_classes, seed, phi=0.0):
    np.random.seed(seed)
    angles = np.linspace(phi, 2 * np.pi + phi, n_classes, endpoint=False)
    centers = np.column_stack((np.cos(angles), np.sin(angles))) * 10
    data = make_blobs(
        n_samples=n_samples,
        centers=centers,
...
def centroids_animation(
    X,
    centroids,
    labels,
    max_frames: Optional[int] = None,
    point_darkening=True,
    marker_size=100,
    t_interval=200,
):
    n_classes, n_centroids, n_features = centroids.shape

    # find ranges of the data
    Xx = np.concatenate((X, centroids.reshape(-1, centroids.shape[2])), axis=0)
    x_min, x_max = Xx[:, 0].min(), Xx[:, 0].max()
    y_min, y_max = Xx[:, 1].min(), Xx[:, 1].max()
    x_margin = (x_max - x_min) * 0.1  # add a 10% margin
    ...
    # each class has: 1. a point, 2. a line that connects the points
    current_points, lines = [], []
    for i in range(n_classes):
        current_point = ax.scatter([], [], color=f"C{i}", marker="o", s=marker_size)
        (line,) = ax.plot([], [], color=f"C{i}", alpha=0.7)  # line
        current_points.append(current_point)
        lines.append(line)

    max_frames = n_centroids if max_frames is None else max_frames
    max_frames = min(max_frames, n_centroids)

    def update(frame_num):
        scatter.set_color([f"C{i}" for i in labels[frame_num]])
        for i in range(n_classes):  # Loop over each class
            x = centroids[i, : frame_num + 1, 0]
            y = centroids[i, : frame_num + 1, 1]
            # Update the current point for class i
            data = np.stack([x, y], axis=1)
            current_points[i].set_offsets(data)
            base_color = np.array(to_rgba(f"C{i}")[:3])
            if point_darkening:
                # Age: newest point has age = 0, oldest = frame_num
                ages = frame_num - np.arange(frame_num + 1)
                darkness = np.clip(1 - ages / max_frames, 0, 1)
                colors = np.outer(darkness, base_color)
            else:
                colors = np.tile(base_color, (frame_num + 1, 1))
            current_points[i].set_color(colors)
            lines[i].set_data(x, y)  # Update the line for class i
        return current_points + lines

    # Show the animation
    anim = animation.FuncAnimation(
        fig, update, frames=max_frames, interval=t_interval, repeat=False
    )
    display(HTML(anim.to_jshtml()))  # Display the video
    plt.close(fig)